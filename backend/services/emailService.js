...
const generateJitsiLink = (patientName, doctorName, appointmentDate, appointmentTime) => {
  ...
  const roomName = `MediConnect-${patientName.replace(/\s+/g, '')}-${doctorName.replace(/\s+/g, '')}-${new Date(appointmentDate).toISOString().split('T')[0]}-${appointmentTime.replace(':', '')}`
...
const sendAppointmentApprovalEmail = async (patientEmail, patientName, doctorName, scheduledDate, scheduledTime, doctorResponse, meetingType) => {
  const subject = 'Appointment Approved - MediConnect'
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>If you need to reschedule or have any questions, please contact us through the MediConnect platform.</p>
...
const sendDoctorConfirmationEmail = async (doctorEmail, doctorName, patientName, scheduledDate, scheduledTime, meetingType) => {
  ...
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">
      ...
      <p>You can manage your appointments through the MediConnect doctor portal.</p>
...
const sendAppointmentRejectionEmail = async (patientEmail, patientName, doctorName, reason) => {
  const subject = 'Appointment Update - MediConnect'
  const htmlContent = `
    <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;">

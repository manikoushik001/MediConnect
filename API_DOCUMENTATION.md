...
## Application URLs
- **Frontend (Production)**: `https://mediconnect.netlify.app/`
- **Frontend (Development)**: `http://localhost:3000`

## API Base URL
- **Development**: `http://localhost:5001/api`
- **Production**: `https://mediconnect-backend.onrender.com/api`
...
```json
{
  "message": "MediConnect Backend API is running!",
  "status": "healthy",
  "version": "1.0.0",
  "timestamp": "2024-01-15T10:00:00.000Z"
}
...
```javascript
// Connect to socket
const socket = io('https://mediconnect-backend.onrender.com')
...
- `https://mediconnect.netlify.app` (production)
...
```bash
# Login
curl -X POST https://mediconnect-backend.onrender.com/api/auth/login \
  -H "Content-Type: application/json" \
  -d '{"username":"john_doe","password":"password123"}'

# Get appointments (with token)
curl -X GET https://mediconnect-backend.onrender.com/api/appointments/history \
  -H "Authorization: Bearer YOUR_JWT_TOKEN"

# Book appointment
curl -X POST https://mediconnect-backend.onrender.com/api/appointments/book \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN" \
  -d '{
    "doctorMedicalId":"DOC001",
...
# Submit feedback
curl -X POST https://mediconnect-backend.onrender.com/api/feedback \
  -H "Content-Type: application/json" \
  -H "Authorization: Bearer YOUR_JWT_TOKEN" \
  -d '{
    "doctorMedicalId":"DOC001",
...
response = requests.post(
    "https://mediconnect-backend.onrender.com/api/auth/login",
    json=login_data
)
...
appointments = requests.get(
    "https://mediconnect-backend.onrender.com/api/appointments/history",
    headers=headers
)
...
```json
{
  ...
  "values": [
    {
      "key": "base_url",
      "value": "https://mediconnect-backend.onrender.com/api"
    },
    {
      "key": "token",
      "value": "{{auth_token}}"
